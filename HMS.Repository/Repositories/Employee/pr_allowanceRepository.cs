...
{
    public static class pr_allowanceRepository
    {
        public static PaginationResult Pagination(this IRepository<pr_allowance> repository, decimal CompanyID, int CurrentPageNo, int RecordPerPage, string VisibleColumnInfo, string SortName, string SortOrder, string SearchText, bool IgnorePaging = false)
        {
            var objResult = new PaginationResult();
            try
            {
                var PFilter = Utility.SetPaginationFilter(CurrentPageNo, RecordPerPage, VisibleColumnInfo, SortName, SortOrder, SearchText);
                Expression<Func<pr_allowance, bool>> predicate = (e => e.CompanyID == CompanyID);
                ...
                IQueryable<pr_allowance> filteredData = repository.Queryable().Where(predicate);
                ...
                if (IgnorePaging)
                {
                    ...
                    objResult.DataList = filteredData.Include(x => x.sys_drop_down_value).Select(s => new
                    {
                        ...
                        AllowanceValue = s.AllowanceValue,
                        s.AllowanceType
                    })
                    .ToList().Select(z => new
                    {
                        z.ID,
                        z.AllowanceName,
                        AllowanceValue = z.AllowanceType == "P" ? (currency + ": " + String.Format("{0:n0}", z.AllowanceValue) + "%") : (currency + ": " + String.Format("{0:n0}", z.AllowanceValue)),
                    })
                    .ToList<object>();
                }
                else
