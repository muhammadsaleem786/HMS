...
{
    public static class pur_Sale_hold_mfRepository
    {
        public static PaginationResult Pagination(this IRepository<pur_sale_hold_mf> repository, decimal CompanyID, int CurrentPageNo, int RecordPerPage, string VisibleColumnInfo, string SortName, string SortOrder, string SearchText, bool IgnorePaging = false)
        {
            var objResult = new PaginationResult();
            try
            {
                var PFilter = Utility.SetPaginationFilter(CurrentPageNo, RecordPerPage, VisibleColumnInfo, SortName, SortOrder, SearchText);
                Expression<Func<pur_sale_hold_mf, bool>> predicate = (e => e.CompanyID == CompanyID);
                bool DisplayNo, DisplayReference, DisplayDate, DisplayCompanyName;
                ...
                IQueryable<pur_sale_hold_mf> filteredData = repository.Queryable().Where(predicate);
                ...
                Expression<Func<pur_sale_hold_mf, string>> orderingFunction = (c =>
                ...
                if (IgnorePaging)
                {
                    var result = filteredData.Include(x => x.pur_sale_hold_dt).Include(x => x.adm_company)
                   .Select(s => new
                   {
                       s.ID,
                       Invoiceno = s.ID,
                ...
                {
                    var result = filteredData.Include(x => x.pur_sale_hold_dt).Include(a => a.adm_company)
                   .Select(s => new
                   {
                       s.ID,
                       Invoiceno = s.ID,
