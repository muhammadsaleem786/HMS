...
@Component({
    selector: 'SaleByPatientRptForm',
    templateUrl: 'SaleByPatientRptForm.html',
    moduleId: module.id,
    providers: [ReportService],
})
export class SaleByPatientRptForm implements OnInit {
    ...
    public Form1: FormGroup; public totalCost: 0; public totalSale: 0; public totalQty: 0;
    ...
    RunReport() {
        ...
        if (this.model.PatientName == null || this.model.PatientName == "")
            this.model.PatientId = null;
        this.reportservice.ItemRpt(this.model).then(m => {
            var result = JSON.parse(m._body);
            if (result.IsSuccess) {
                const feeList = result.ResultSet.FeeList;
                this.DataList = Array.isArray(feeList)[0] ? feeList : Object.values(feeList)[0];
                const totalRecord = this.DataList.filter(a => a.Type == "Total");
                this.totalCost = totalRecord[0].CostPrice;
                this.totalSale = totalRecord[0].SalePrice;
                this.totalQty = totalRecord[0].Qty;
                this.DataList = this.DataList.filter(a => a.Type != "Total");              
