...
{
    [JwtAuthentication]
    public class pur_sale_hold_mfController : ApiController, IERPAPIInterface<pur_sale_hold_mf>, IDisposable
    {
        ...
        private readonly Ipur_sale_hold_mfService _service;
        ...
        public pur_sale_hold_mfController(IUnitOfWorkAsync unitOfWorkAsync,
         Ipur_sale_hold_mfService Service,
        ...
        public ResponseInfo GetByIdParam(string Id)
        {
            ...
            {
                var CompanyID = Request.CompanyID();
                var result = _service.Queryable().Where(e => e.CompanyID == CompanyID && e.ID.ToString() == Id).Include(x => x.pur_sale_hold_dt).FirstOrDefault();
                var itemids = result.pur_sale_hold_dt.Select(a => a.ItemID).ToList();
        ...
        public async Task<ResponseInfo> Save(pur_sale_hold_mf Model)
        {
            var objResponse = new ResponseInfo();
            try
            {
                ...
                decimal CompanyID = Request.CompanyID();
                List<pur_sale_hold_dt> purorderitem = new List<pur_sale_hold_dt>();
                purorderitem.AddRange(Model.pur_sale_hold_dt);
                ...
                Model.pur_sale_hold_dt = null;
                Model.adm_user_mf = null;
                Model.adm_company = null;
                Model.adm_user_mf1 = null;
                _service.Insert(Model);
                //insert pur_sale_hold_dt
                decimal saleItemID = 1;
                if (_pur_sale_hold_dtService.Queryable().Count() > 0)
                    saleItemID = _pur_sale_hold_dtService.Queryable().Max(e => e.ID) + 1;
                foreach (pur_sale_hold_dt item in purorderitem)
                {
                    item.ID = saleItemID;
                    ...
                    _pur_sale_hold_dtService.Insert(item);
                    saleItemID++;
                }
                try
                {
        ...
        [HttpGet]
        [ActionName("Update")]
        public async Task<ResponseInfo> Update(pur_sale_hold_mf Model)
        {
            var objResponse = new ResponseInfo();
            try
            {
                ...
                pur_sale_hold_mf saleobj = _service.Queryable().Where(a => a.ID == Model.ID).FirstOrDefault();
        ...
        [HttpGet]
        [ActionName("UpdateSaleHoldStatus")]
        public async Task<ResponseInfo> UpdateSaleHoldStatus(string Id)
        {
            var objResponse = new ResponseInfo();
            try
            {
                ...
                pur_sale_hold_mf saleobj = _service.Queryable().Where(a => a.ID.ToString() == Id).FirstOrDefault();
                saleobj.HoldStatus = false;
                saleobj.ObjectState = ObjectState.Modified;
                _service.Update(saleobj);
                try
                {
                    await _unitOfWorkAsync.SaveChangesAsync();
                    objResponse.Message = MessageStatement.Update;
        ...
        [HttpGet]
        [ActionName("SaleHoldDelete")]
        public async Task<ResponseInfo> SaleHoldDelete()
        {
            var objResponse = new ResponseInfo();
            try
            {
                decimal CompanyID = Request.CompanyID();
                var models = await _service.Queryable().Where(e => e.HoldStatus == false || DbFunctions.TruncateTime(e.CreatedDate) < DbFunctions.TruncateTime(DateTime.Now) && e.CompanyID == CompanyID).ToListAsync();
                ...
                var idsToDelete = models.Select(e => e.ID).ToList();
                var relatedItems = await _pur_sale_hold_dtService.Queryable()
                .Where(e => idsToDelete.Contains(e.SaleID) && e.CompanyID == CompanyID)
                .ToListAsync();

                _pur_sale_hold_dtService.DeleteRange(relatedItems);
                var mainItems = await _service.Queryable()
                .Where(e => idsToDelete.Contains(e.ID))
                .ToListAsync();

                _service.DeleteRange(mainItems);
                await _unitOfWorkAsync.SaveChangesAsync();
        ...
        public async Task<ResponseInfo> Delete(string Id)
        {
            ...
            try
            {
                ...
                pur_sale_hold_mf Model = _service.Queryable().Where(e => e.ID == id && e.CompanyID == CompanyID).FirstOrDefault();
                ...
                var DelAllsale_sale_item = _pur_sale_hold_dtService.Queryable().Where(e => e.SaleID.ToString() == Id && e.CompanyID == CompanyID).ToList();
                foreach (var obj in DelAllsale_sale_item)
                {
                    obj.ObjectState = ObjectState.Deleted;
                    _pur_sale_hold_dtService.Delete(obj);
                }
