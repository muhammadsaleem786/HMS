...
namespace HMS.Web.API.Areas.Employee
{
    ...
    [JwtAuthentication]
    public class adm_company_locationController : ApiController, IERPAPIInterface<adm_company_location>, IDisposable
    {
        private readonly Iadm_company_locationService _service;
        private readonly Ipr_employee_mfService _pr_employee_mfService;
        private readonly IUnitOfWorkAsync _unitOfWorkAsync;

        public adm_company_locationController(IUnitOfWorkAsync unitOfWorkAsync, Iadm_company_locationService Service, Ipr_employee_mfService pr_employee_mfService)
        {
        ...
        public async Task<ResponseInfo> Save(adm_company_location Model)
        {
            ...
            try
            {
                ...
                adm_company_location CompLocation = _service.Queryable().Where(x => x.CompanyID == CompanyID && x.LocationName.ToLower() == Model.LocationName.ToLower()).FirstOrDefault();
                ...
                {
                    objResponse.IsSuccess = false;
                    objResponse.ErrorMessage = Model.LocationName + " already exist. Please enter a different name and try again.";
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<ResponseInfo> SaveLocationAndReturnList(adm_company_location Model)
        {
            var objResponse = new ResponseInfo();
            try
            {
                ...
                if (CompLocation == null)
                {
                    ...
                    try
                    {
                        ...
                        {
                            ID = Model.ID,
                            LocationList = _service.Queryable().Where(x => x.CompanyID == CompanyID).Select(x => new { ID = x.ID, LocationName = x.LocationName }).ToList(),
                        };
        ...
        public ResponseInfo GetById(string Id)
        {
            ...
            try
            {
                ...
                objResponse.ResultSet = _service.Queryable()
                   ...
                   .Select(x => new { x.ID, x.LocationName, x.CompanyID, x.CityDropDownID, x.CountryDropDownID, x.Address, x.CityID, x.CountryID, x.ZipCode, Employees = x.pr_employee_mf.Count() }).FirstOrDefault();
            }
            catch (Exception ex)
            {
                objResponse.IsSuccess = false;
        ...
        [HttpGet]
        [ActionName("Update")]
        public async Task<ResponseInfo> Update(adm_company_location Model)
        {
            ...
            try
            {
                ...
                decimal CompanyID = Request.CompanyID();
                adm_company_location CompLocation = _service.Queryable().Where(x => x.CompanyID == CompanyID && x.ID != Model.ID && x.LocationName.ToLower() == Model.LocationName.ToLower()).FirstOrDefault();
                if (CompLocation == null)
                {
        ...
        public async Task<ResponseInfo> Delete(string Id)
        {
            ...
            try
            {
                ...
                List<adm_company_location> Models = _service.Queryable().Where(e => e.CompanyID == CompanyID && IdList.Contains(e.ID)).ToList();
