...
{
    [JwtAuthentication]
    public class pr_allowanceController : ApiController, IERPAPIInterface<pr_allowance>, IDisposable
    {
        private readonly Ipr_allowanceService _service;

        private readonly Isys_drop_down_valueService _sys_drop_down_valueService;
        private readonly IUnitOfWorkAsync _unitOfWorkAsync;

        public pr_allowanceController(IUnitOfWorkAsync unitOfWorkAsync, Ipr_allowanceService Service, Isys_drop_down_valueService sys_drop_down_valueService)
        {
        ...
        public async Task<ResponseInfo> Save(pr_allowance Model)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(Model.AllowanceName))
                {
                    ...
                    Model.AllowanceName = DropDownValue.Value;
                }
                pr_allowance Allowance = _service.Queryable().Where(x => x.CompanyID == CompanyID && x.AllowanceName.ToLower() == Model.AllowanceName.ToLower()).FirstOrDefault();
                if (Allowance == null)
                {
                    decimal ID = 1;
                ...
                {
                    objResponse.IsSuccess = false;
                    objResponse.ErrorMessage = Model.AllowanceName + " already exist. Please enter a different name and try again.";
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<ResponseInfo> SaveAndReturnList(pr_allowance Model)
        {
            var objResponse = new ResponseInfo();
            try
            {
                ...
                if (Allowance == null)
                {
                    ...
                    try
                    {
                        ...
                        {
                            ID = Model.ID,
                            AllowancesList = _service.Queryable()
        ...
        public async Task<ResponseInfo> SaveAndReturnAllowance(pr_allowance Model)
        {
            var objResponse = new ResponseInfo();
            try
            {
        ...
        [HttpGet]
        [ActionName("Update")]
        public async Task<ResponseInfo> Update(pr_allowance Model)
        {
            ...
            try
            {
                ...
                pr_allowance Allowance = _service.Queryable().Where(x => x.CompanyID == CompanyID && x.ID != Model.ID && x.AllowanceName.ToLower() == Model.AllowanceName.ToLower()).FirstOrDefault();
                if (Allowance == null)
                {
                    Model.ModifiedBy = Request.LoginID();
                    Model.ModifiedDate = Request.DateTimes();
        ...
        public async Task<ResponseInfo> Delete(string Id)
        {
            ...
            try
            {
                ...
                List<pr_allowance> Models = _service.Queryable().Where(e => e.CompanyID == CompanyID && IdList.Contains(e.ID)).ToList();
