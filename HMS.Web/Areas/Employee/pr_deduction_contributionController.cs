...
{
    [JwtAuthentication]
    public class pr_deduction_contributionController : ApiController, IERPAPIInterface<pr_deduction_contribution>, IDisposable
    {
        private readonly Ipr_deduction_contributionService _service;
        private readonly IUnitOfWorkAsync _unitOfWorkAsync;

        public pr_deduction_contributionController(IUnitOfWorkAsync unitOfWorkAsync, Ipr_deduction_contributionService Service)
        {
        ...
        [ActionName("Save")]
        [HttpPost]
        public async Task<ResponseInfo> Save(pr_deduction_contribution Model)
        {
            ...
            try
            {
                ...
                pr_deduction_contribution deduction_contribution = _service.Queryable().Where(x => x.CompanyID == CompanyID && x.DeductionContributionName.ToLower() == Model.DeductionContributionName.ToLower() && x.Category == Model.Category).FirstOrDefault();
                if (deduction_contribution == null)
                {
                ...
                {
                    objResponse.IsSuccess = false;
                    objResponse.ErrorMessage = Model.DeductionContributionName + " already exist. Please enter a different name and try again.";
                }
        ...
        [ActionName("SaveDedContandReturnDedCont")]
        [HttpPost]
        public async Task<ResponseInfo> SaveDedContandReturnDedCont(pr_deduction_contribution Model)
        {
            var objResponse = new ResponseInfo();
            try
            {
        ...
        [HttpGet]
        [ActionName("Update")]
        public async Task<ResponseInfo> Update(pr_deduction_contribution Model)
        {
            ...
            try
            {
                ...
                decimal CompanyID = Request.CompanyID();
                pr_deduction_contribution deduction_contribution = _service.Queryable().Where(x => x.CompanyID == CompanyID && x.ID != Model.ID && x.DeductionContributionName.ToLower() == Model.DeductionContributionName.ToLower() && x.Category == Model.Category).FirstOrDefault();
                if (deduction_contribution == null)
                {
                    Model.ModifiedBy = Request.LoginID();
                    Model.ModifiedDate = Request.DateTimes();
        ...
        public async Task<ResponseInfo> Delete(string Id)
        {
            ...
            try
            {
                ...
                List<pr_deduction_contribution> Models = _service.Queryable().Where(e => e.CompanyID == CompanyID && IdList.Contains(e.ID)).ToList();
